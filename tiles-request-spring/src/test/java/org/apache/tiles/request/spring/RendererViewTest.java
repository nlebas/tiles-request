...
public class RendererViewTest {
    ...
    public void setUp() {
        ...
        locale = Locale.ITALY;
        expect(servletContext.getInitParameter(anyString())).andReturn(null).anyTimes();
        expect(springContext.getServletContext()).andReturn(servletContext).anyTimes();
        expect(springContext.getBean(anyString(), anyClass())).andThrow(new NoSuchBeanDefinitionException("mock")).anyTimes();
        expect(request.getAttribute(RequestContext.WEB_APPLICATION_CONTEXT_ATTRIBUTE)).andReturn(springContext)
                .anyTimes();
        expect(request.getAttribute(anyObject(String.class))).andReturn(null).anyTimes();
    ...
    private static String anyString() {
    	return anyObject(String.class);
    }
    
    @SuppressWarnings("unchecked")
    private static Class<Object> anyClass() {
    	return anyObject(Class.class);
    }
}
